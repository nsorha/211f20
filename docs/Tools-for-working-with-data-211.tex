...
Then use the \texttt{alpha()} function, which is part of the psych package, to compute Cronbachs alpha of all if the variables in the cmaq object. The \texttt{psych::} tells r to use the alpha in the psych package (there is also an alpha in the tidyverse package).

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{psych}\OperatorTok{::}\KeywordTok{alpha}\NormalTok{(cmaq)}
\end{Highlighting}
\end{Shaded}
...
\StringTok{  }\KeywordTok{select}\NormalTok{(cmaq, bpm) }\OperatorTok{%>%}\StringTok{ }
