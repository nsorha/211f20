...
**NOTE:** Please open a new script and call it lab 4 (or week 4).
...
[Here is the data.](https://docs.google.com/spreadsheets/d/1Ty_kgqdsTM8f1h-lJ-HjkpYqXGc7hAAyP_F9vq-tdEI/edit?usp=sharing)
...
Then use the `alpha()` function, which is part of the psych package, to compute Cronbachs alpha of all if the variables in the cmaq object. The `psych::` tells r to use the alpha in the psych package (there is also an alpha in the tidyverse package).
...
```{r}
ggplot(cmaqpilot, aes(x=cmaq, y=bpm)) +
  geom_point() 
```
...
CMAQ scores were positively related to students' heart rate (r = .95, p < .001, CI.95 = .90 to .97).
